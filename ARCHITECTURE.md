...
#### TagSettingsDialog (AppSidebar.jsx)
标签管理弹窗组件，集成在侧边栏中，支持二级嵌套标签的完整管理。

**布局结构：**
- 头部："标签管理台"标题 + 简洁描述 + 关闭按钮
- 新增分类：一级类目输入区（短输入框 + 按钮）
- 分类卡片：每个一级分类独立卡片（柔和边框 + 微阴影）
- 底部：取消/保存按钮

**文案细节：**
- 标题：标签管理台
- 副标题：在这里整理一级分类和二级标签，最后统一提交保存。
- 一级类目按钮：一级类目 → 输入框占位符"例如：项目类型"

**视觉细节：**
- 卡片：圆角 26px，边框 1px white/70，柔和背景
- 一级标签：圆角胶囊，moss 色调，系统标签灰色
- 二级标签：圆角卡片，白色背景，简洁阴影
- 整体留白：px-8 大边距，通透呼吸感
...
│           ├── DetailPanel.jsx
│           ├── GalleryCard.jsx
│           ├── TagSettingsDialog    # 标签管理弹窗（内嵌于 AppSidebar）
│           └── ui/          # 基础 UI 组件
├── scripts/                  # 构建脚本
│   ├── build-mac-icon.mjs
│   ├── build-mas.mjs
